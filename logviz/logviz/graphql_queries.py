...
class Query(graphene.ObjectType):
    ...
    all_sampling_events = graphene.List(
        SamplingEvent, run_id=graphene.String(required=True)
    )
    ...
    @timing
    def resolve_sampling_events(
        self, info, run_id: str, sample_id: str
    ) -> list[SamplingEvent]:
        return _get_sampling_events(run_id, sample_id)

    @timing
    def resolve_sample_metrics(
        self, info, run_id: str, sample_id: str
    ) -> Optional[SampleMetrics]:
        return _get_sample_metrics(run_id, sample_id)

    @timing
    def resolve_sample_page(
        self, info, run_id: str, page_id: int
    ) -> Optional[SamplePage]:
        ...
        sample_page = _get_sample_page_from_sample_id(
            run_id, target_sample_id, backend_page_id
        )
...
def _get_sample_page_from_sample_id(
    run_id: str, sample_id: str, page_id: int
) -> SamplePage:
