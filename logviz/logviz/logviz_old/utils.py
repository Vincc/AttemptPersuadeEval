...
def build_trajectories(log_lines: list[AbstractLogLine]):
    ...
    for page in pages:
        ...
        for sample in reversed(samples):
            ...
            for message in sample.prompt.data:
                ...
                if message["content"] in seen_texts:
                    ...
                    combined_sample_messages.append(
                        {
                            "role": role_with_i,
                            "content": f"[duplicate of {duplicate_message}]",
                        }
                    )
                else:
                    combined_sample_messages.append(
            ...
            assert (
                len(sample.sampled) == 1
            ), f"Expected 1 sampled text, got {len(sample.sampled)}"
            combined_sample_messages.append(
...
def build_pages(
    ...
    for sample_id in keys:
        ...
        assert all(
            isinstance(s, LogSampling) for s in samples
        ), f"samples is {type(samples)}"
    ...
    for page in pages:
        ...
        if target_keywords:
            for keyword in target_keywords:
                if all(
                    not sample.prompt.contains(keyword) for sample in sorted_samples
                ):
                    include_page = False
